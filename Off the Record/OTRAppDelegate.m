...
@synthesize preController = _preController;
@synthesize fswindow = _fswindow;
@synthesize fsblankwindow = _fsblankwindow;

- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
    [audioSession setActive:NO error: nil];
    [audioSession setCategory:AVAudioSessionCategoryAudioProcessing error:nil];// withOptions:AVAudioSessionCategoryOptionMixWithOthers error:nil];
...
*/
    ...
    NSString *outputStoreName = @"Chatterbox.sqlite";
    ...
    self.fsblankwindow = [[UIWindow alloc] initWithFrame:UIScreen.mainScreen.bounds];
    [self.fsblankwindow setOpaque:YES];
    [self.fsblankwindow setBackgroundColor:[UIColor whiteColor]];
    [self.fsblankwindow setHidden:NO];

    self.fswindow = [[UIWindow alloc] initWithFrame:UIScreen.mainScreen.bounds];
    [self.fswindow setHidden:NO];
    ...
    self.preController = [[PreController alloc] initWithNibName:nil bundle:nil];
    self.preController.view.tag = 1000000;


    self.window.rootViewController = rootViewController; //self.preController;
    //self.fswindow.rootViewController = self.preController;
    [self.fswindow addSubview:self.preController.view];
  
    [self.window makeKeyAndVisible];

  UIView* blankView = [[UIView alloc] initWithFrame:UIScreen.mainScreen.bounds];
  [blankView setOpaque:YES];
  [blankView setBackgroundColor:[UIColor whiteColor]];
  [blankView setHidden:NO];
  [self.fsblankwindow addSubview:blankView];
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
  ...
  self.backgroundTask = [application beginBackgroundTaskWithExpirationHandler:^{
    ...
    NSLog(@"Application going away now");
    });
  }];
}
